...
import { fetchBangumiLite, fetchBangumiPointsDetail, searchBangumiByKey } from '../utils/api'
import { planRoute } from '../utils/routePlanner'
import { generateItineraryWithOllama } from '../utils/ollama'
...
export const useAppStore = defineStore('app', () => {
  ...
  async function generateLibraryItinerary(options = {}) {
    const selected = librarySelected.value
    if (selected.length === 0) return
    loading.value = true
    try {
      const result = planRoute(selected, libraryDays.value, defaultCenter.value, options)
      libraryItinerary.value = result
      ...
      let animeNames = [...new Set(points.map(p => p.anime).filter(Boolean))]
      if (animeNames.length === 0) animeNames = ['动漫']

      const routeInfo = {
        totalDistance: result.reduce((sum, day) => sum + (day.distance || 0), 0),
        totalDuration: 0
      }

      try {
        const aiResult = await generateItineraryWithOllama(
          animeNames,
          libraryDays.value,
          points,
          routeInfo
        )
        aiItinerary.value = aiResult
        ElMessage.success('AI 行程规划生成成功！')
      } catch (aiError) {
        console.error('AI 行程生成失败:', aiError)
        ElMessage.warning('AI 行程生成失败，将仅显示地图路线')
      }
    } catch (e) {
      error.value = e.message || '路线规划失败'
    } finally {
